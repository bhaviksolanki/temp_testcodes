...
import com.ms.datawise.distn.annotation.RetryMethod; 
...
@Aspect
public class DatazoneTransferAspect implements Ordered{
	
	private static Logger LOGGER = LoggerFactory.getLogger(DatazoneTransferAspect.class); 
	private int order = -1;

	@Around (value ="@annotation (retryMethod)", argNames="retryMethod")
	public Object datazoneRetry (final ProceedingJoinPoint pjp,
		final RetryMethod retryMethod) throws Throwable {
		
		final Integer maxRetries = retryMethod.retryAttempts(); 
		Integer retryAttempt = 0;
		Object result = null;
		Class<? extends Throwable> [] retryExceptions = retryMethod.retryExceptions();
		while (retryAttempt < maxRetries) {
		try {
				result = pjp.proceed(); 
				break;
				} catch (final Exception exception) {
				Thread.sleep (retryMethod.sleepInterval());
